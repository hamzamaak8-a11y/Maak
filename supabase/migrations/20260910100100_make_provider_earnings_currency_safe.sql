...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE p.id=v_uid AND p.role='provider' AND p.account_status='active' AND pp.verification_status='approved'
  ) THEN RAISE EXCEPTION 'forbidden'; END IF;

  SELECT count(*)::integer INTO v_completed
  FROM public.bookings b WHERE b.provider_id=v_uid AND b.status='completed';

  SELECT count(DISTINCT b.currency)::integer, min(b.currency), coalesce(sum(b.price),0)
    INTO v_currency_count, v_earnings_currency, v_earnings
  FROM public.bookings b
  WHERE b.provider_id=v_uid AND b.status='completed' AND b.payment_status='paid' AND b.price IS NOT NULL;

  IF v_currency_count <> 1 THEN
    v_earnings := NULL;
    v_earnings_currency := NULL;
  END IF;
  ...
  FROM (
    ...
    ORDER BY b.service_date ASC LIMIT 5
  ) x;

  SELECT coalesce(jsonb_agg(x.item ORDER BY x.created_at DESC),'[]'::jsonb) INTO v_activity
  FROM (
    SELECT jsonb_build_object('id',n.id,'type',n.type,'title',n.title,'body',n.body,'is_read',n.is_read,'created_at',n.created_at,'metadata',n.metadata) item,n.created_at
    FROM public.notifications n WHERE n.user_id=v_uid ORDER BY n.created_at DESC LIMIT 5
  ) x;
