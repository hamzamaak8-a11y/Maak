...
export default function ProviderDetailScreen() {
  ...
  return <ScrollView style={styles.root} contentContainerStyle={styles.content}>
    ...
    <View style={styles.hero}>
      ...
      <Text style={styles.job}>{provider.job}</Text>
      <Text style={styles.meta}>{provider.city}{provider.rating ? ` · ★ ${provider.rating}` : ''}{provider.reviews ? ` · ${provider.reviews} تقييم` : ''}</Text>
    </View>
    <View style={styles.card}><Text style={styles.title}>الخدمات</Text><View style={styles.services}>{provider.services.map((service) => <View style={styles.chip} key={service}><Text style={styles.chipText}>{service}</Text></View>)}</View></View>
    {provider.intro ? <View style={styles.card}><Text style={styles.title}>نبذة</Text><Text style={styles.body}>{provider.intro}</Text></View> : null}
