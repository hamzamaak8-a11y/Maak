...
export default function AccountScreen() {
  ...
  return <View style={styles.root}>
    ...
    <View style={styles.profileCard}>
      ...
      <Text style={styles.name}>{profile?.full_name ?? 'حساب Maak'}</Text>
      <Text style={styles.email}>{user?.email ?? 'غير مسجل الدخول'}</Text>
      {profile?.city ? <Text style={styles.city}>{profile.city}</Text> : null}
    </View>
    <View style={styles.actions}>
      <Pressable style={styles.action}><Text style={styles.actionText}>اللغة · العربية</Text></Pressable>
      <Pressable style={styles.action}><Text style={styles.actionText}>الإشعارات</Text></Pressable>
