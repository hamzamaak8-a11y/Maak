...
async function resetE2ETransactions(userIds) {
  if (!userIds.length) return;
  assertOk(
    await supabase.from("reviews").delete().or(`customer_id.in.(${userIds.join(",")}),provider_id.in.(${userIds.join(",")})`),
    "reset E2E reviews",
  );
  assertOk(
    await supabase.from("bookings").delete().or(`customer_id.in.(${userIds.join(",")}),provider_id.in.(${userIds.join(",")})`),
    "reset E2E bookings",
  );
}
...
async function ensureHiddenReview(providerId, customerId, listingId) {
  const booking = assertOk(
    await supabase.from("bookings").insert({
      ...
      service_description: `${MARKER} hidden review fixture`,
...
async function seed() {
  ...
  await resetE2ETransactions([admin.id, provider.id, customer.id]);
...
async function cleanup() {
  ...
  await resetE2ETransactions(userIds);
