...
type Props = { profile: Profile; email: string; onSaved: (profile: Profile) => void };

export default function CustomerProfileCard({ profile, email, onSaved }: Props) {
  const { t } = useLanguage();
  const [editing, setEditing] = useState(false);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [form, setForm] = useState<CustomerProfileUpdate>({ full_name: profile.full_name, phone: profile.phone, city: profile.city, avatar_url: profile.avatar_url });

  useEffect(() => { setForm({ full_name: profile.full_name, phone: profile.phone, city: profile.city, avatar_url: profile.avatar_url }); }, [profile]);
  ...
  return (
    <section className="customer-card customer-profile-card" aria-labelledby="customer-profile-heading">
      <div className="customer-card-head">
        <div><span className="section-kicker">{t("customer.profileKicker")}</span><h2 id="customer-profile-heading">{t("customer.profileTitle")}</h2></div>
        {!editing ? <button className="ghost-button" onClick={() => setEditing(true)}><Edit3 size={16} /> {t("customer.editProfile")}</button> : null}
      </div>
      <div className="customer-profile-hero">
        ...
        <div><h3>{profile.full_name || t("customer.profileFallback")}</h3><p>{t("account.roleCustomer")}</p></div>
      </div>
      {editing ? (
        <div className="customer-form-grid">
          ...
          <p className="customer-form-note">{t("customer.photoUrlNote")}</p>
        ...
        <div className="customer-detail-grid">
          ...
          <div><span>{t("common.email")}</span><strong>{email || "—"}</strong></div>
