...
export async function listMarketplace():Promise<{rows:AdminMarketplaceListing[];total:number}>{const {data,error,count}=await supabase.from("providers").select("id,name,job,city,distance,rating,reviews,image,available,provider_profile_id,published_at",{count:"exact"}).eq("listing_kind","real").not("published_at","is",null).order("published_at",{ascending:false}).limit(50);if(error)throw fail(error,"adm.loadMarketplaceFail");return{rows:(data??[]) as AdminMarketplaceListing[],total:count??0}}
