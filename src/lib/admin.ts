...
export const DOC_LABELS: Record<string, string> = {
  national_id: "onb.docNationalId", profile_photo: "onb.docProfilePhoto", professional_document: "adm.doc_professional_document", other: "adm.doc_other",
};
...
type RawAppRow = { id: string; profession: string | null; service_category: string | null; bio: string | null; experience_years: number | null; verification_status: VerificationStatus; rejection_reason: string | null; created_at: string; updated_at: string };
...
function toApp(r: RawAppRow, p: RawProfile | undefined): AdminApplication {
  return { id: r.id, profession: r.profession, service_category: r.service_category, bio: r.bio, experience_years: r.experience_years, verification_status: r.verification_status, rejection_reason: r.rejection_reason, created_at: r.created_at, updated_at: r.updated_at, full_name: p?.full_name ?? null, phone: p?.phone ?? null, city: p?.city ?? null, avatar_url: p?.avatar_url ?? null, account_status: p?.account_status ?? "active" };
}
export async function listApplications(status: VerificationStatus): Promise<AdminApplication[]> {
  const { data: rows, error } = await supabase.from("provider_profiles").select("id,profession,service_category,bio,experience_years,verification_status,rejection_reason,created_at,updated_at").eq("verification_status", status).order("updated_at", { ascending: false });
  if (error) throw msg(error, "adm.loadAppsFail");
  const list = (rows ?? []) as RawAppRow[];
  if (!list.length) return [];
  const ids = list.map((r) => r.id);
  const { data: profs, error: perr } = await supabase.from("profiles").select("id,full_name,phone,city,avatar_url,account_status").in("id", ids);
...
export async function approveProvider(id: string): Promise<void> { const { error } = await supabase.rpc("admin_approve_provider", { target: id }); if (error) throw msg(error, "adm.errApprove"); }
export async function rejectProvider(id: string, reason: string): Promise<void> { const trimmed = reason.trim(); if (!trimmed) throw new Error("adm.errReasonRequired"); const { error } = await supabase.rpc("admin_reject_provider", { target: id, reason: trimmed }); if (error) throw msg(error, "adm.errReject"); }
export async function setAccountStatus(id: string, status: "active" | "suspended"): Promise<void> { const { error } = await supabase.rpc("admin_set_account_status", { target: id, new_status: status }); if (error) throw msg(error, "adm.accountActionFail"); }
export async function cancelBooking(id: string, reason: string): Promise<void> { const { error } = await supabase.rpc("admin_cancel_booking", { target: id, reason: reason.trim() || null }); if (error) throw msg(error, "adm.bookingActionFail"); }

export type AdminCustomer = { id: string; role: string; full_name: string | null; phone: string | null; city: string | null; created_at: string; account_status: "active" | "suspended" };
export type AdminBooking = { id: string; customer_id: string; provider_id: string; provider_listing_id: number | null; service_category: string; service_date: string | null; location_text: string | null; status: string; rejection_reason: string | null; created_at: string; customer_name: string | null; provider_name: string | null };
export const ADMIN_PAGE_SIZE = 50;

export async function listCustomers(): Promise<{ rows: AdminCustomer[]; total: number }> {
  const { data, error, count } = await supabase.from("profiles").select("id,role,full_name,phone,city,created_at,account_status", { count: "exact" }).eq("role", "customer").order("created_at", { ascending: false }).limit(ADMIN_PAGE_SIZE);
...
export async function listBookings(): Promise<{ rows: AdminBooking[]; total: number }> {
  const { data, error, count } = await supabase.from("bookings").select("id,customer_id,provider_id,provider_listing_id,service_category,service_date,location_text,status,rejection_reason,created_at,customer_name", { count: "exact" }).order("created_at", { ascending: false }).limit(ADMIN_PAGE_SIZE);
