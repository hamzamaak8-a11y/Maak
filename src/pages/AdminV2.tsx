  ...
  return <div className="m2-shell">
    ...
    <aside className={`m2-sidebar ${collapsed ? "collapsed" : ""} ${mobileOpen ? "mobile-open" : ""}`}>
      <div className="m2-sidebar-inner">
        ...
        {!collapsed && <button className="m2-search" onClick={() => document.getElementById("m2-global-search")?.focus()}><Search size={16}/><span>Search</span><kbd>⌘ K</kbd></button>}
        ...
        <div className="m2-sidebar-bottom"><div className="m2-admin-user"><span className="m2-avatar">MK</span>{!collapsed&&<div><b>Maak Admin</b></div>}</div></div>
    ...
    <main className="m2-main">
      ...
      {notice && <div className="m2-notice" role="status">{notice}</div>}
      {loading ? <section className="m2-placeholder"><span className="m2-placeholder-icon"><Activity size={22} /></span><h1>Loading…</h1></section> : page === "dashboard" ? <>
        <section className="m2-hero"><div><h1>Maak <em>Control Center.</em></h1><div className="m2-hero-actions"><button className="m2-btn primary" onClick={()=>go("verification")}><ShieldCheck size={16}/> Review queue · {pendingCount}</button><button className="m2-btn light" onClick={()=>go("bookings")}><CalendarCheck2 size={16}/> Bookings · {data.bookingsTotal}</button></div></div><div className="m2-hero-mini-grid"><div><span>BOOKINGS</span><b>{data.bookingsTotal}</b></div><div><span>CUSTOMERS</span><b>{data.customersTotal}</b></div><div><span>PROVIDERS</span><b>{data.providersApproved}</b></div><div><span>LISTINGS</span><b>{data.marketplaceTotal}</b></div></div></section>
