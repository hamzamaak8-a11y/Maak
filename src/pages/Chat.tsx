...
export default function Chat({ conversationId }: { conversationId?: string }) {
  ...
  async function loadConversations() {
    ...
    try {
      ...
      } else if (!rows.some((row) => row.conversation_id === conversationId)) {
        setActiveId(null);
      }
    } catch (err) {
      setError(err instanceof Error ? t(err.message) : t("chat.loadFail"));
  ...
  function selectConversation(id: string) {
    setActiveId(id); setMobileList(false); setMessageError(null); navigate(`/chat/${id}`);
  }
  ...
  if (authLoading || (loading && !conversations.length)) {
    return <main className="screen"><div className="pdetail-loading"><Loader2 className="spin" size={24}/><p>{t("common.loading")}</p></div></main>;
  }
  if (!user) return null;
  ...
  return (
    <main className="chat-page">
      <div className="chat-shell">
        <div style={{ display: mobileList ? "block" : undefined }} className={mobileList ? "" : "chat-mobile-hidden"}>
          <ConversationList conversations={conversations} activeId={activeId} onSelect={selectConversation}/>
        </div>
        {activeId && activeConversation && !mobileList ? (
          <section className="chat-main" aria-label={t("chat.messages")}>
            <header className="chat-main-header">
              ...
              <span className={`chat-realtime-dot ${realtimeStatus === "SUBSCRIBED" ? "" : "off"}`} aria-label={realtimeStatus ?? ""}/>
            </header>
            {messageError && <div className="chat-error" role="alert">{messageError}</div>}
            {realtimeStatus === "CHANNEL_ERROR" || realtimeStatus === "TIMED_OUT" ? <div className="chat-error" role="status">{t("chat.realtimeError")}</div> : null}
          ...
          <section className="chat-main"><div className="chat-message-empty"><MessageCircle size={30}/><strong>{t("chat.selectConversation")}</strong></div></section>
        ) : null}
      </div>
      {error && <div className="chat-error" role="alert">{error} <button className="mini-button" type="button" onClick={() => void loadConversations()}><RefreshCw size={14}/>{t("common.retry")}</button></div>}
    </main>
  );
}
