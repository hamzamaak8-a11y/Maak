...
function ActionButton({ label, onClick, tone="default", disabled=false, icon }: { label:string; onClick:()=>void; tone?:"default"|"danger"|"success"; disabled?:boolean; icon:React.ReactNode }) {
...
export default function AdminModeration() {
  ...
  const load = useCallback(async()=>{
    ...
    } catch (e) { setNotice(e instanceof Error ? e.message : "Unable to load moderation data"); }
  ...
  async function act(id:string, fn:()=>Promise<void>, success:string){ setBusy(id); try{ await fn(); setNotice(success); await load(); }catch(e){setNotice(e instanceof Error?e.message:"Action failed");}finally{setBusy(null);} }
  ...
  return <section className="adm-center">
    ...
    {notice&&<div className="adm-notice" role="status">{notice}<button type="button" aria-label="Dismiss" onClick={()=>setNotice("")}><XCircle size={16}/></button></div>}
    ...
    <nav className="adm-tabs" aria-label="Moderation sections">{([['users','Users'],['providers','Providers'],['bookings','Bookings'],['reviews','Reviews']] as const).map(([id,label])=><button key={id} className={tab===id?"active":""} type="button" onClick={()=>setTab(id)}>{label}<span>{id==='users'?users.length:id==='providers'?providers.length:id==='bookings'?bookings.length:reviews.length}</span></button>)}</nav>
    {loading?<div className="adm-loading"><RotateCcw size={20} className="adm-spin"/>Loading moderation data…</div>:
      tab==='users'?<UserTable rows={filteredUsers} busy={busy} onSuspend={(id)=>void act(id,()=>admin.suspendUser(id),"User suspended")} onActivate={(id)=>void act(id,()=>admin.activateUser(id),"User activated")} onDelete={(id)=>{if(confirmDelete("this suspended user"))void act(id,()=>admin.deleteUser(id),"User deleted");}}/>:
