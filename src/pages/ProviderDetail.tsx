        ...
        <section className="pdetail-identity">
          ...
          <div className="pdetail-info">
            ...
            <ul className="pdetail-trust">
              <li><span className={`t-ico ${provider.rating && Number(provider.rating) > 0 ? "gold" : ""}`}><Star size={15} fill="currentColor" /></span><span className="t-val">{provider.rating && Number(provider.rating) > 0 ? <><b>{provider.rating}</b> <small>{t("pd.reviewsCount", { n: provider.reviews })}</small></> : <b>{t("common.new")}</b>}</span></li>
