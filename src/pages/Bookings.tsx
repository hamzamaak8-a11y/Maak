...
export default function Bookings() {
  ...
  useEffect(() => {
      ...
      new Map(
        bookings
          ...
          .map((booking) => {
            ...
            return provider?.provider_profile_id ? [provider.provider_profile_id, true] : null;
          })
          .filter((entry): entry is [string, true] => Boolean(entry)),
      ).keys(),
    );
    if (!targets.length) {
    ...
    void Promise.all(targets.map((providerId) => getProviderReviews(providerId, 50, 0)))
      .then((results) => {
        if (cancelled) return;
        const ids = new Set<string>();
        results.forEach((result) => result.reviews.forEach((review) => ids.add(review.booking_id)));
        setReviewedBookingIds(ids);
      })
      .catch(() => {
        if (!cancelled) setReviewedBookingIds(new Set());
      });
    return () => { cancelled = true; };
  }, [bookings]);
  ...
  if (!user) {
    ...
    return (
      <main className="screen bookings-screen">
        <div className="page-title"><div><span className="section-kicker">{t("bookings.sub")}</span><h1>{t("bookings.title")}</h1></div></div>
        <div className="auth-gate">
          <h3>{t("bookings.loginRequired")}</h3><p>{t("bookings.loginRequiredBody")}</p>
          <div className="actions"><button className="primary" onClick={() => goAuth("/login")}>{t("adminLogin.signIn")}</button><button className="ghost-button" onClick={() => goAuth("/register")}>{t("auth.createAccountBtn")}</button></div>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="screen bookings-screen">
      ...
      <div className="seg-tabs" role="tablist">
        {TABS.map((item) => <button key={item.key} className={"seg-tab" + (tab === item.key ? " active" : "")} role="tab" aria-selected={tab === item.key} onClick={() => setTab(item.key)}>{item.label}</button>)}
      </div>
          ...
          return (
            <div className="booking-card" key={booking.id}>
              ...
              <div className="booking-main">
                <span className={statusClass(booking.status)}>{t(BOOKING_STATUS_LABELS[booking.status])}</span>
                <h3>{t(booking.service_category)}</h3><p>{provider ? provider.name : t("account.roleProvider")}</p>
                ...
                {canCancel ? (cancelId === booking.id ? (
                  <div className="cta-row" style={{ marginTop: 10 }}><button className="secondary mini-button" onClick={() => void doCancel(booking.id)} disabled={busy}><X size={14} /> {t("bk.confirmCancel")}</button><button className="ghost-button mini-button" onClick={() => setCancelId(null)} disabled={busy}>{t("bk.backOut")}</button></div>
                ) : <button className="ghost-button mini-button" style={{ marginTop: 8 }} onClick={() => setCancelId(booking.id)}><X size={14} /> {t("bk.cancelRequest")}</button>) : null}
              </div>
            </div>
          );
        })
