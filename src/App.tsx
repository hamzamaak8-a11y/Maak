...
import { Header, MobileNav, SideRail } from "./components/navigation";
...
function AppSplash() { const { t, lang } = useLanguage(); return <div className="maak-splash" aria-label={t("common.loading")}><span className="maak-splash-mark"><MaakMark size={76}/></span><span className="maak-splash-word" aria-hidden="true">{lang === "ar" ? "معك" : "Maak"}</span></div>; }
function CustomerShell() { const { path } = useRouter(); const bookingParams=matchPath("/provider/:id/booking",path); const providerParams=bookingParams?null:matchPath("/provider/:id",path); const chatParams=matchPath("/chat/:conversationId",path); let content:ReactNode; if(path==="/login")content=<Login/>; else if(path==="/register")content=<Register/>; else if(path==="/forgot-password")content=<ForgotPassword/>; else if(path==="/reset-password")content=<ResetPassword/>; else if(bookingParams)content=<BookingFlow id={Number(bookingParams.id)}/>; else if(providerParams)content=<ProviderDetail id={Number(providerParams.id)}/>; else if(path==="/bookings")content=<Bookings/>; else if(path==="/help")content=<Help/>; else if(path==="/onboarding")content=<Onboarding/>; else if(path==="/account")content=<Account/>; else if(path==="/discover")content=<Discover/>; else if(path==="/chat")content=<Chat/>; else if(chatParams)content=<Chat conversationId={chatParams.conversationId}/>; else if(path==="/")content=<Home/>; else content=<NotFound/>; const isProviderScreen=path.startsWith("/provider"); const isAuthScreen=path==="/login"||path==="/register"||path==="/forgot-password"||path==="/reset-password"; return <div className="app"><div className="shell">{!isAuthScreen&&<SideRail path={path}/>}<div className="shell-col">{!isAuthScreen&&<Header path={path}/>}<main className="app-main" key={path}>{content}</main></div></div>{!isAuthScreen&&!isProviderScreen&&<MobileNav path={path}/>}<ToastViewport/></div>; }
function AdminGate(){const{path,navigate}=useRouter();const{user,loading,profile,profileLoading}=useAuth();const isLoginPage=path==="/admin/login";const ready=!loading&&!profileLoading;const isAdmin=ready&&!!user&&profile?.role==="admin"&&profile?.account_status!=="suspended";useEffect(()=>{if(!ready)return;if(isLoginPage){if(isAdmin)navigate("/admin");}else if(!isAdmin)navigate("/admin/login");},[ready,isAdmin,isLoginPage,navigate]);if(loading)return <AppSplash/>;if(isLoginPage)return ready&&isAdmin?<AppSplash/>:<AdminLogin/>;if(!isAdmin)return <AppSplash/>;return <AdminV2/>;}
