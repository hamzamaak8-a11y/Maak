...
function CustomerShell() {
  ...
  const chatParams = matchPath("/chat/:conversationId", path);
  ...
  else if (chatParams) content = <Chat conversationId={chatParams.conversationId}/>;
  else content = <Home/>;
...
function AdminGate() {
  const { path, navigate } = useRouter(); const { user, loading, profile, profileLoading } = useAuth(); const isLoginPage = path === "/admin/login"; const ready = !loading && !profileLoading; const isAdmin = ready && !!user && profile?.role === "admin" && profile?.account_status !== "suspended";
  useEffect(() => { if (!ready) return; if (isLoginPage) { if (isAdmin) navigate("/admin"); } else if (!isAdmin) navigate("/admin/login"); }, [ready, isAdmin, isLoginPage, navigate]);
  if (loading) return <AppSplash/>; if (isLoginPage) return ready && isAdmin ? <AppSplash/> : <AdminLogin/>; if (!isAdmin) return <AppSplash/>; return <AdminV2/>;
}
